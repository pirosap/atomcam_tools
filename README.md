...
ATOMCam2の無線LAN機能を無効化し、USBで接続する有線LANアダプタ経由での利用に変更するための変更を加えています。
無線LAN機能は完全に停止させており、電波を出力しません。
利用には、DHCPサーバが動作している有線LANネットワークに接続する必要があります。

現状、Realtek RTL8152を使用するUSB-Ethernetアダプタでのみ動作します。
Amazon イーサネットアダプタをUSBで接続する有線LANアダプタとして動作確認を行っています。
PoE環境にてPoEスプリッタを利用すれば、PoEによる利用が可能です。
